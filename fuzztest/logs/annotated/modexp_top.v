...
%000000	output [15:0] exp_addr;
	output [2047:0] exp_m, exp_exp, exp_n;
%000001	output exp_step, exp_valid;
...
%000000	wire sel_reg_start = addr == EXP_REG_START;
%000000	wire sel_reg_state = addr == EXP_REG_STATE;
%000000	wire sel_reg_addr  = {addr[15:1],1'b0} == EXP_REG_ADDR;
%000000	wire sel_reg_m     = {addr[15:8],8'b0} == EXP_REG_M;
%000000	wire sel_reg_exp   = {addr[15:8],8'b0} == EXP_REG_EXP;
%000000	wire sel_reg_n     = {addr[15:8],8'b0} == EXP_REG_N;
	
	// state register.
%000000	reg [1:0] exp_reg_state;
	...
	// state predicates.
%000001	wire exp_state_idle = exp_reg_state == EXP_STATE_IDLE;
%000000	wire exp_state_operate = exp_reg_state == EXP_STATE_OPERATE;
%000000	wire exp_state_wait = exp_reg_state == EXP_STATE_WAIT;
%000000	wire exp_state_write_data = exp_reg_state == EXP_STATE_WRITE_DATA;
	
	// consider writes only when we are in the idle states. other writes ignored.
%000000	wire wren = wr && exp_state_idle;
%000000	wire start_op = sel_reg_start && data_in[0] && wren;
	
	// address register.
%000000	wire [15:0] exp_reg_opaddr;
%000000	wire [7:0] exp_addr_dataout;
 ...
 000391	wire [7:0] bigend_addr = 8'hff - addr[7:0];
	
	// m register.
	wire [2047:0] exp_reg_m;
%000000	wire [7:0] exp_m_dataout;
...
%000000	wire [7:0] exp_exp_dataout;
...
%000000	reg [7:0] byte_counter;
%000000	wire reset_byte_counter = start_op;
%000000	wire incr_byte_counter = xram_ack;
%000000	wire [7:0] byte_counter_next = reset_byte_counter ? 0                :
	                               incr_byte_counter  ? byte_counter + 1 :
	                               byte_counter;
	// is this the last byte?
%000000	wire last_byte_acked = byte_counter == 255 && xram_ack;
	...
	// next state logic.
%000000	wire [2:0] exp_reg_state_next_idle = start_op ? EXP_STATE_OPERATE : EXP_STATE_IDLE;
%000001	wire [2:0] exp_reg_state_next_operate = EXP_STATE_WAIT;
%000002	wire [2:0] exp_reg_state_next_wait = exp_valid ? EXP_STATE_WRITE_DATA : EXP_STATE_WAIT;
%000002	wire [2:0] exp_reg_state_next_write_data = last_byte_acked ? EXP_STATE_IDLE : EXP_STATE_WRITE_DATA;
	
%000000	wire [2:0] exp_reg_state_next = 
