...
%000000	output [15:0] sha_rdaddr, sha_wraddr, sha_len;
%000000	output sha_step;
%000001	output sha_core_assumps_valid;
	...
	// state register.
%000000	reg [2:0]  sha_reg_state;
	wire [2:0] sha_state = sha_reg_state;
	
	// state predicates.
%000001	wire sha_state_idle       = sha_reg_state == SHA_STATE_IDLE;
%000000	wire sha_state_read_data  = sha_reg_state == SHA_STATE_READ_DATA;
%000000	wire sha_state_op1        = sha_reg_state == SHA_STATE_OP1;
%000000	wire sha_state_op2        = sha_reg_state == SHA_STATE_OP2;
%000000	wire sha_state_write_data = sha_reg_state == SHA_STATE_WRITE_DATA;
	
	// register selector predicates.
%000000	wire sel_reg_start = addr == SHA_REG_START;
%000000	wire sel_reg_state = addr == SHA_REG_STATE;
%000000	wire sel_reg_rd_addr  = {addr[15:1], 1'b0} == SHA_REG_RD_ADDR;
%000000	wire sel_reg_wr_addr  = {addr[15:1], 1'b0} == SHA_REG_WR_ADDR;
%000000	wire sel_reg_len      = {addr[15:1], 1'b0} == SHA_REG_LEN;
%000000	wire wren = wr && sha_state_idle;
...
%000000	wire [2:0] sha_state_next;
	
%000000	wire [2:0] sha_state_next_idle;
%000001	wire [2:0] sha_state_next_read_data;
%000002	wire [2:0] sha_state_next_op1;
%000002	wire [2:0] sha_state_next_op2;
%000001	wire [2:0] sha_state_next_write_data;
...
%000000	reg [5:0] byte_counter;
%000000	wire [5:0] byte_counter_next;
%000000	wire [5:0] byte_counter_next_rw;
...
%000000	reg [15:0] block_counter;
%000000	wire [15:0] block_counter_next;
	assign block_counter_next = 
	    sha_state_idle                   ? 16'b0              : 
	    sha_state_op2 && sha_more_blocks ? block_counter + 64 : block_counter;
	
	// Are we reading the last byte?
%000001	wire reading_last_byte = (byte_counter == 6'd63) || (bytes_read_next == sha_reg_len);
%000000	wire read_last_byte_acked = reading_last_byte && xram_ack;
	
	// Are we done reading or do we have more blocks?
%000000	wire sha_more_blocks = sha_core_digest_valid && (reg_bytes_read <  sha_reg_len);
%000000	wire sha_finished    = sha_core_digest_valid && (reg_bytes_read >= sha_reg_len);
	...
	// Writing logic.
%000000	wire writing_last_byte = byte_counter == 6'd19;
%000000	wire write_last_byte_acked = writing_last_byte && xram_ack;
	
%000000	wire [7:0] data_out_state,
%000000	           data_out_rd_addr,
%000000	           data_out_wr_addr,
%000000	           data_out_len;
	...
	// rd address register.
%000000	wire [15:0] sha_reg_rd_addr;
	...
	// wr address register.
%000000	wire [15:0] sha_reg_wr_addr;
...
%000000	wire [15:0] sha_reg_len;
	...
	// Active low reset.
%000000	wire sha_core_rst_n = !rst; 
	// Set to one to start hashing the first block of data.
%000000	wire sha_core_init = sha_state_op1 && sha_core_ready_r && (block_counter == 0);
	// Set to one to start hashing the next blocks of data.
%000000	wire sha_core_next = sha_state_op1 && sha_core_ready_r && (block_counter != 0);
	// Output from core signalling that it is ready for init/next to be set to 1.
%000001	wire sha_core_ready;
	// flopped version.
%000001	reg sha_core_ready_r;
	// Output from core signalling that the hash result is ready.
%000000	wire sha_core_digest_valid;
	// Register which holds the data to be hashed.
	reg [511:0] sha_core_block;
	// Hash output.
%000000	wire [159:0] sha_core_digest;
	// Hash output flops.
%000000	reg [159:0] sha_reg_digest;
	// The MUX which selects the input to the flops.
%000000	wire [159:0] sha_reg_digest_next;
	assign sha_reg_digest_next = sha_core_digest_valid ? sha_core_digest : sha_reg_digest;
	    
%000002	reg sha_core_assumps_valid_reg;
%000002	wire sha_core_assumps_valid_reg_next = 
