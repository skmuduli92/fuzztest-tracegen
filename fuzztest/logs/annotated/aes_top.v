	...
	// 8051 <=> AES
 000564	input clk, wr, stb, rst;
 000214	input [7:0] data_in;
 000416	input [15:0] addr;
 000256	output [7:0] data_out;
 000152	output ack;
%000004	output in_addr_range;
	// AES <=> XRAM
 000246	output [15:0] xram_addr;
 000359	output [7:0] xram_data_out;
 000804	input [7:0] xram_data_in;
 000256	input xram_ack;
 000012	output xram_stb;
%000008	output xram_wr;
	// verif output.
 000020	output [1:0] aes_state;
%000004	output [15:0] aes_addr, aes_len;
%000114	output [127:0] aes_ctr, aes_key0, aes_key1;
 000020	output aes_step;
...
%000004	wire sel_reg_start  = addr == AES_REG_START;
%000004	wire sel_reg_state  = addr == AES_REG_STATE;
%000000	wire sel_reg_keysel = addr == AES_REG_KEYSEL;
%000002	wire sel_reg_addr   = {addr[15:1], 1'b0} == AES_REG_ADDR;
%000002	wire sel_reg_len    = {addr[15:1], 1'b0} == AES_REG_LEN;
%000002	wire sel_reg_ctr    = {addr[15:4], 4'b0} == AES_REG_CTR;
%000002	wire sel_reg_key0   = {addr[15:4], 4'b0} == AES_REG_KEY0;
%000000	wire sel_reg_key1   = {addr[15:4], 4'b0} == AES_REG_KEY1;
	
	// state register.
 000020	reg [1:0]  aes_reg_state;
	...
	// state predicates.n
%000005	wire aes_state_idle = aes_reg_state == AES_STATE_IDLE;
%000008	wire aes_state_read_data = aes_reg_state == AES_STATE_READ_DATA;
%000008	wire aes_state_operate = aes_reg_state == AES_STATE_OPERATE;
%000008	wire aes_state_write_data = aes_reg_state == AES_STATE_WRITE_DATA;
	
	// consider writes only when we are in the idle state. other writes ignored.
 000076	wire wren = wr && aes_state_idle;
%000004	wire start_op = sel_reg_start && data_in[0] && wren;
	
	// key select register
%000000	reg aes_reg_keysel;
%000000	wire aes_reg_keysel_next = sel_reg_keysel && wren ? data_in[0] : aes_reg_keysel;
	   
	// address register.
%000003	wire [15:0] aes_reg_opaddr;
 000309	wire [7:0] aes_addr_dataout;
	...
	// length register.
%000001	wire [15:0] aes_reg_oplen;
 000102	wire [7:0] aes_len_dataout;
	...
	// ctr register.
%000050	wire [127:0] aes_reg_ctr;
 000439	wire [7:0] aes_ctr_dataout;
	...
	// key0 register.
%000064	wire [127:0] aes_reg_key0;
 000368	wire [7:0] aes_key0_dataout;
	...
	// key1 register.
%000000	wire [127:0] aes_reg_key1;
%000000	wire [7:0] aes_key1_dataout;
...
%000007	reg [15:0] operated_bytes_count;
%000007	wire [15:0] operated_bytes_count_next = 
	    start_op                                ? 0                         : 
...
%000003	reg [15:0] block_counter;
%000003	wire [15:0] block_counter_next = start_op    ? 0                    : 
	                                 more_blocks ? (block_counter + 16) :
	                                 block_counter;
	
	// keep track of the current byte being read/written.
 000240	reg [3:0] byte_counter;
%000004	wire reset_byte_counter = start_op;
 000256	wire incr_byte_counter = xram_ack;
 000240	wire [3:0] byte_counter_next = reset_byte_counter ? 0                :
	                               incr_byte_counter  ? byte_counter + 1 :
	                               byte_counter;
	// is this the last byte in the block?
 000016	wire last_byte_acked = byte_counter == 15 && xram_ack;
	// one-hot signal that denotes if there are there more blocks to go to.
%000004	wire more_blocks = last_byte_acked && aes_state_write_data &&
	...
	// next state logic.
%000004	wire [1:0] aes_reg_state_next_idle = start_op ? AES_STATE_READ_DATA : AES_STATE_IDLE;
 000033	wire [1:0] aes_reg_state_next_read_data = last_byte_acked ? AES_STATE_OPERATE : AES_STATE_READ_DATA;
%000002	wire [1:0] aes_reg_state_next_operate = AES_STATE_WRITE_DATA;
 000030	wire [1:0] aes_reg_state_next_write_data = 
 ...
 000020	wire [1:0] aes_reg_state_next = 
...
%000402	reg [127:0] mem_data_buf;
%000402	wire [127:0] mem_data_buf_next;
...
%000053	wire [127:0] aes_ctr_v = aes_reg_ctr + {112'b0, block_counter};
 002640	wire [127:0] aes_out;
 003042	wire [127:0] encrypted_data = aes_out ^ mem_data_buf;
%000064	wire [127:0] aes_curr_key = aes_reg_keysel ? aes_reg_key1 : aes_reg_key0;
	aes_128 aes_128_i (
	    .clk        (clk),
	    .state      (aes_ctr_v),
	    .key        (aes_curr_key),
...
%000204	reg  [127:0] encrypted_data_buf;
%000204	wire [127:0] encrypted_data_buf_next = 
